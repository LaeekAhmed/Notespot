...
router.post("/", ClerkExpressRequireAuth(), (req, res) => {
   ...
   let upload = multer({
      storage,
      limits: { fileSize: 1000000 * 100, fieldSize: 2 * 1024 * 1024 },
   }).single("userFile");
   upload(req, res, async (err) => {
      ...
      if (err) {
         ...
         return res.status(500).json({
            error: err.message + ", try to use a image with size <= 500 kb ",
         });
      }
      ...
      try {
         ...
         try {
            ...
            console.log(
               "unabled to delete file from bucket while creating doc: ",
               deleteError
            );
         }
